...
2. After fitting a random forest, use cross-validation and hyperparameter tuning on the random forest. Finalize the best-fitting model and compute predictive fit. Have you improved on the previous best-fitting model?
...
Let's create a workflow for a random forest then update it for subsequent models.
...
By tuning the hyperparameters, we get the same predictive accuracy of ~72%, so the cross-validation did not net us an improvement in model performance.
